...
import {
  IDENTITY, PROJECTS, EMPLOYMENT, STACK, STACK_FRAME, LOCALES, CERTS, EDUCATION,
  CONTACT, EGG, hex, PALETTE,
} from '../content.js'
...
export class Director {
  constructor({ ui, stage, audio, pointer, cursor, env }) {
    this.ui = ui; this.stage = stage; this.audio = audio
    this.pointer = pointer; this.cursor = cursor; this.env = env
    this.index = 0; this.busy = false; this.adapted = false
    this.activeEl = null

    this.frames = this._defineFrames()
    this._buildScenes()
    this._buildHud()
    this._buildDossier()
    this._bindInput()
  ...
  _defineFrames() {
    ...
    PROJECTS.forEach((p) =>
      f.push({ id: 'proj:' + p.key, label: p.name, accent: p.accent, mode: p.mode, root: p.root, proj: p }))
    f.push({ id: 'stack', label: 'STACK + LOCALES', accent: PALETTE.chameleon, mode: 'major', root: 261.63 })
    f.push({ id: 'status', label: 'STATUS', accent: PALETTE.chameleon, mode: 'major', root: 261.63 })
    return f
  }
  ...
  _buildScenes() {
    ...
    this.sections = {}
    this.sections.identity = this._identity()
    this.sections.index = this._index()
    PROJECTS.forEach((p, i) => { this.sections['proj:' + p.key] = this._project(p, i) })
    this.sections.stack = this._stack()
    this.sections.status = this._status()
  ...
  _buildHud() {
    const hud = el('div', 'hud'); this.ui.appendChild(hud)
    ...
    hud.appendChild(ctr)
    this.muteBtn = ctr.querySelector('#muteBtn')
    ctr.querySelector('#dossierBtn').addEventListener('click', () => this.toggleDossier())
    ...
    this.railIdx = rail.querySelector('#railIdx')
    this.railName = rail.querySelector('#railName')
    this.railFill = rail.querySelector('#railFill')
    this.nodes = [...rail.querySelectorAll('.node')]
    this.nodes.forEach((n) => n.addEventListener('click', () => this.goTo(+n.dataset.i)))
  }
  ...
  _toggleMute() { this.audio.toggleMute(); this._reflectMute() }
  ...
  _buildDossier() {
    ...
    this.dossier = d
    d.querySelector('.d-close').addEventListener('click', () => this.toggleDossier(false))
  }
  ...
  _bindInput() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'm' || e.key === 'M') { this._toggleMute(); return }
    ...
    window.addEventListener('pointerdown', (e) => {
      if (e.target.closest('.hud, .dossier, button, a')) return
    ...
    this.scenesRoot.addEventListener('click', (e) => {
      ...
      if (jump) { const idx = this.frames.findIndex((f) => f.id === jump.dataset.jump); if (idx >= 0) this.goTo(idx) }
      const copy = e.target.closest('[data-copy]')
      if (copy) this._copy(copy)
      if (e.target.closest('.restart')) this.goTo(0)
    })
  }
  ...
  _copy(btn) {
    ...
    this.audio.bell && this.audio.bell(660)
    const hint = btn.querySelector('.copy__hint'); const old = hint.textContent
  ...
  enterIdentity() {
    ...
    this._syncHud()
    this._applyFrame(this.frames[0], true)
  }
  ...
  goTo(i) {
    ...
    const from = this.frames[this.index], to = this.frames[i]
    const fromEl = this.sections[from.id], toEl = this.sections[to.id]
    const kinetic = (from.proj || to.proj)
    this.busy = true
    this._idle && this._idle.kill()
    this.audio.glitch()
    ...
    tl.add(() => this._enterScene(to, dir), 0.34)
  ...
  _enterScene(f, dir) {
    ...
    const num = q('.pj-num'), name = q('.pj-name'), meta = q('.pj-meta'), cine = q('.pj-cine .gambetta')
    const cmd = q('.pj-deploy__cmd'), status = q('.pj-deploy__status')
    const bar = q('.pj-bar__fill'), uline = q('.pj-uline')
    const desc = q('.pj-desc'), pts = sec.querySelectorAll('.pj-points li'), chips = sec.querySelectorAll('.chip')
    ...
    this._scramble(name, p.name, 0.7)
    if (cine) this._scramble(cine, p.title, 0.6)
    this._type(cmd, `$ deploy ${p.key} --prod`, 0.55)
    gsap.to(bar, {
      width: '100%', duration: 0.9, delay: 0.4, ease: 'power1.inOut',
      onComplete: () => { status.textContent = live ? '● RUNNING' : '✓ DEPLOYED' },
    })

    this._idle = gsap.to(num, { rotation: d * 1.5, y: 16, scale: 1.03, duration: 4.5, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 0.9 })
  }

  _type(elm, text, dur = 0.5) {
    if (!elm) return
    clearInterval(elm._ti)
    ...
    elm._ti = setInterval(() => {
      ...
      if (f >= total) { clearInterval(elm._ti); elm.textContent = text }
    }, 1000 / 60)
  }

  _scramble(elm, text, dur = 0.7) {
    if (!elm) return
    clearInterval(elm._sc)
    const glyphs = '!<>-_\\/[]{}=+*^?#01'
    const total = Math.max(1, Math.round(dur * 60))
    let f = 0
    elm._sc = setInterval(() => {
      ...
      elm.textContent = out
      if (f >= total) { clearInterval(elm._sc); elm.textContent = text }
    }, 1000 / 60)
  }
  ...
  _applyFrame(f, instant) {
    const accentN = this.adapted ? f.accent : 0x9a958c
    if (!this._acc) this._acc = unpack(accentN)
    const tgt = unpack(accentN)
    gsap.to(this._acc, {
      r: tgt.r, g: tgt.g, b: tgt.b, duration: instant ? 0 : 1.0, ease: 'power2.out',
      onUpdate: () => {
        const r = Math.round(this._acc.r), g = Math.round(this._acc.g), b = Math.round(this._acc.b)
        document.documentElement.style.setProperty('--accent', `rgb(${r},${g},${b})`)
        this.stage.setAccent((r << 16) | (g << 8) | b)
      },
    })
    this.audio.setChapter({ mode: f.mode, root: f.root })

    if (this.stage.titleGroup) {
  ...
  _syncHud() {
    ...
    this.nodes.forEach((n, i) => { n.classList.toggle('is-active', i <= this.index); n.classList.toggle('is-here', i === this.index) })
  }

  parallax(p) {
    if (!this.activeEl || this.busy) return
    const body = this.activeEl.querySelector('.pj-body')
    const numWrap = this.activeEl.querySelector('.pj-num-wrap')
...
function unpack(n) { return { r: (n >> 16) & 255, g: (n >> 8) & 255, b: n & 255 } }
